...
> **关于毛玻璃（backdrop-filter）**：我们曾尝试注入 CSS 为 DSH 容器叠加 `backdrop-filter: blur()` 以形成真·玻璃，但 `backdrop-filter` 会创建新的包含块（containing block），导致 DSH 内 `position: fixed` 的模态弹窗（设置面板等）定位失效、被「困」在容器里。为保证布局绝不回归，已**移除**毛玻璃注入——纯 token + 弥散光方案足以实现清透高级感，且零布局风险。

> 配色负责「气质」，弥散光负责「质感」。两者配齐，才是完整的高级感。
