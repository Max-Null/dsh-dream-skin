...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"bg2.remove": "清除壁纸",
			"modal.title": "弹窗不透明度",
			"modal.hint": "控制选项弹窗 / 选项卡的底填充透明度——越高越不透明、文字越清晰；越低越能透出背后的内容。"
		};
		...
		const en = {
			...
			"modal.hint": "Controls how opaque the popup / option-card fill is — higher keeps text crisp, lower lets the content behind show through."
		};
		...
		const ja = {
			...
			"modal.hint": "オプションのポップアップ / カードの塗り不透明度を調整します。高いほど不透明で文字がはっきりし、低いと背後が透けます。"
		};
		...
		const ko = {
			...
			"bg2.remove": "배경화면 지우기",
			"modal.title": "팝업 불투명도",
			"modal.hint": "옵션 팝업 / 카드 채우기 불투명도를 조절합니다. 높을수록 불투명해져 텍스트가 선명하고, 낮으면 배경이 비쳐 보입니다."
		};
		...
		const es = {
			...
			"modal.hint": "Controla la opacidad del relleno de las ventanas emergentes / tarjetas de opciones: más alta mantiene el texto nítido, más baja deja ver el contenido de atrás."
		};
		...
		const fr = {
			...
			"modal.hint": "Contrôle l'opacité du remplissage des fenêtres / cartes d'options : plus élevée, le texte reste net ; plus basse, le contenu derrière transparaît."
		};
		...
		const de = {
			...
			"modal.hint": "Regelt die Fülldeckkraft von Popups / Optionskarten – höher hält den Text klar, niedriger lässt den Hintergrund durchscheinen."
		};
		...
		const ru = {
			...
			"modal.hint": "Регулирует непрозрачность заливки всплывающих окон / карточек с параметрами: выше — текст чётче, ниже — просвечивает фон."
		};
		...
		function ensureMaterialStyle() {
			...
			const parts = [
				...
				"  background: transparent;",
				"}",
				// --- Dropdown / popup menu readability (issue: menus see-through) ---
				// DSH maps every dropdown/popup menu's background (`--dsw-specific-menu`)
				// to `--dsw-alias-bg-layer-3`. The skins deliberately set layer-3 to a
				// near-transparent white (e.g. rgba(255,255,255,.085) on the dark skins)
				// for the "清透" glass aesthetic, so ALL menus (model picker, input
				// triggers, option lists, etc.) rendered almost see-through and their
				// text was hard to read. Repoint menus at the readable elevated surface
				// `--dsw-alias-bg-layer-2` (the same fill DSH's dialogs/settings use),
				// which the skins keep opaque — text stays crisp on every skin.
				":root, body {",
