...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		const POPUP_OVERRIDE_SOURCE = "dsh-dream-skin:popup-opacity";
		const POPUP_TOKENS = ["--dsw-alias-bg-overlay", "--dsw-specific-menu"];
		let popupOverrideDispose = null;
		function applyModalOverlay(ctx) {
			...
			popupOverrideDispose?.();
			popupOverrideDispose = ctx.theme.overrideTokens(POPUP_OVERRIDE_SOURCE, overrides);
		}
		//#endregion
