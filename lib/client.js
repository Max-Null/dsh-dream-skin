	...
	factory: (require) => {
		...
		function tryImportFromHash(ctx) {
			...
			try {
				...
				// Dedupe like importPack(): a pack already imported (e.g. previously
				// via a share link) must not be pushed into the in-process registry twice.
				if (!importedPacks.some((p) => p && p.id === decoded.id)) {
		...
		function apply(ctx) {
			...
			const localeT = ctx.locale.bind(SETTINGS_NS);
