	...
	factory: (require) => {
		...
		function ensureMaterialStyle() {
			...
			const parts = [
				...
				"  }",
				"}",
				// A second rule (always applied, not @supports-gated) gives the
				// composer root a bottom-scrim so a scrolled-up message / "turn N"
				// monitor row never shows through under the input. It is tinted with
				// the ACTIVE BASE color (not the tip color, not translucent white),
				// so on light skins it exactly matches the page background — no
				// grey/black fog, no dirty gradient — while still shielding the
				// scrolled content behind the input. On deep skins it leans on the
				// deep base and stays subtle too.
				".uV2eYG_root {",
				"  background: linear-gradient(to bottom,",
				"    color-mix(in srgb, var(--dsw-alias-bg-base) 92%, transparent) 0%,",
